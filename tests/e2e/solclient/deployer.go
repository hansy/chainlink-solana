...
import (
	...
	deployercli2 "github.com/smartcontractkit/chainlink-solana/tests/e2e/deployercli"
	access_controller2 "github.com/smartcontractkit/chainlink-solana/tests/e2e/generated/access_controller"
	deviation_flagging_validator2 "github.com/smartcontractkit/chainlink-solana/tests/e2e/generated/deviation_flagging_validator"
	ocr_22 "github.com/smartcontractkit/chainlink-solana/tests/e2e/generated/ocr_2"
...
const (
	...
	AccessControllerStateAccountSize      = uint64(8 + 32 + 1 + 32*32)
	DeviationFlaggingValidatorAccountSize = uint64(8 + 32*4 + 32*128 + 1)
	OCRAccountAccountSize                 = uint64(8 + 352 + 4096 + 8 + 4096 + 8 + 8 + 8 + 1 + 1 + 128*19 + 40*19 + 1 + 32)
	OCRTransmissionsAccountSize           = uint64(4 + 4 + 8192*20)
)
...
type ContractDeployer struct {
	...
	Shell  *deployercli2.DockerShell
}
	...
	err = c.Client.TXAsync(
		...
		[]solana.Instruction{
			...
			deviation_flagging_validator2.NewInitializeInstruction(
				stateAcc.PublicKey(),
				c.Client.Owner.PublicKey(),
				bacPublicKey,
				bacPublicKey,
			).Build(),
		},
		func(key solana.PublicKey) *solana.PrivateKey {
			if key.Equals(c.Client.Owner.PublicKey()) {
				return &c.Client.Owner.PrivateKey
			}
...
func (c *ContractDeployer) addMintToAccInstr(instr *[]solana.Instruction, dest *solana.Wallet, amount uint64) error {
	*instr = append(*instr, token.NewMintToInstruction(
		amount,
		c.Client.MintStateAcc.PublicKey(),
		dest.PublicKey(),
		c.Client.MintAuthority.PublicKey(),
		nil,
	).Build())
	return nil
}
...
func (c *ContractDeployer) DeployLinkTokenContract() (contracts.LinkToken, error) {
	...
	if err := c.Client.addNewAssociatedAccInstr(c.Client.OCRVaultLINKAcc, c.Client.Owner.PublicKey(), &instr); err != nil {
		return nil, err
	}
	if err := c.addMintToAccInstr(&instr, c.Client.OCRVaultLINKAcc, 10000000); err != nil {
		return nil, err
	}
	err := c.Client.TXAsync(
		...
		instr,
		func(key solana.PublicKey) *solana.PrivateKey {
			if key.Equals(c.Client.OCRVaultLINKAcc.PublicKey()) {
				return &c.Client.OCRVaultLINKAcc.PrivateKey
			}
			if key.Equals(c.Client.MintStateAcc.PublicKey()) {
				return &c.Client.MintStateAcc.PrivateKey
			}
			if key.Equals(payer.PublicKey()) {
				return &payer.PrivateKey
			}
			if key.Equals(c.Client.MintAuthority.PublicKey()) {
				return &c.Client.MintAuthority.PrivateKey
			}
			return nil
		},
		payer.PublicKey(),
...
func (c *ContractDeployer) DeployOCRv2(billingControllerAddr string, requesterControllerAddr string, linkTokenAddr string) (contracts.OCRv2, error) {
	...
	payer := c.Client.DefaultWallet
	ocrAccInstruction, err := c.Client.CreateAccInstr(c.Client.OCRStateAcc, OCRAccountAccountSize, programWallet.PublicKey())
	if err != nil {
		return nil, err
	}
	ocrTransmissionsAccInstruction, err := c.Client.CreateAccInstr(c.Client.OCRTransmissionsAcc, OCRTransmissionsAccountSize, programWallet.PublicKey())
	if err != nil {
		return nil, err
	}
	...
	authorities, err := c.Client.generateOCRAuthorities([]string{"vault", "validator"})
	if err != nil {
		return nil, err
	}
	c.Client.Authorities = authorities
	vault := c.Client.Authorities["vault"]
	err = c.Client.TXAsync(
		"Initializing OCRv2",
		[]solana.Instruction{
			ocrAccInstruction,
			ocrTransmissionsAccInstruction,
			ocr_22.NewInitializeInstructionBuilder().
				...
				SetStateAccount(c.Client.OCRStateAcc.PublicKey()).
				SetTransmissionsAccount(c.Client.OCRTransmissionsAcc.PublicKey()).
				SetPayerAccount(payer.PublicKey()).
				SetOwnerAccount(c.Client.Owner.PublicKey()).
				SetTokenMintAccount(linkTokenMintPubKey).
				SetTokenVaultAccount(c.Client.OCRVaultLINKAcc.PublicKey()).
				SetVaultAuthorityAccount(vault.AuthorityPubKey).
		...
		func(key solana.PublicKey) *solana.PrivateKey {
			...
			if key.Equals(c.Client.OCRStateAcc.PublicKey()) {
				return &c.Client.OCRStateAcc.PrivateKey
			}
			if key.Equals(c.Client.Owner.PublicKey()) {
				return &c.Client.Owner.PrivateKey
			}
			if key.Equals(c.Client.OCRTransmissionsAcc.PublicKey()) {
				return &c.Client.OCRTransmissionsAcc.PrivateKey
			}
			return nil
		},
		payer.PublicKey(),
...
// DeployProgram deploy a program on a static key from "keys" dir using CLI
func (c *ContractDeployer) DeployProgram(contractName string) error {
	contractPath := filepath.Join("contracts", contractName)
	contractsKeyFileName := strings.Replace(contractName, ".so", "-keypair.json", -1)
	contractKeyFile := filepath.Join("contracts", contractsKeyFileName)
	log.Debug().
		Str("Path", contractPath).
		Str("KeyPath", contractKeyFile).
		Str("Name", contractName).Msg("Deploying program")
	cmd := fmt.Sprintf("solana -C keys/config.yml deploy %s %s --commitment finalized", contractPath, contractKeyFile)
	log.Debug().Str("CMD", cmd).Msg("Executing cmd")
	if err := c.Shell.RunCMD(cmd); err != nil {
		return err
	}
	return nil
}
...
func (c *ContractDeployer) registerAnchorPrograms() {
	...
	ocr_22.SetProgramID(c.Client.ProgramWallets["ocr2-keypair.json"].PublicKey())
}
...
func (c *Client) generateOCRAuthorities(seeds []string) (map[string]*Authority, error) {
	...
	for _, seed := range seeds {
		auth, nonce, err := c.FindAuthorityAddress(seed, c.OCRStateAcc.PublicKey(), c.ProgramWallets["ocr2-keypair.json"].PublicKey())
		if err != nil {
			return nil, err
		}
